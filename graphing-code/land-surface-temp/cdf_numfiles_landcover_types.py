...
def main():
    ...
    plt.title('CDF of the Number of Files per Day Across Kenya 2018')
        ...
        while current <= end_date_obj:
            ...
            for f in files:
                ...
                if df_bounding_box.empty:
                    ...
                    pass
                else: #increment files for this day that have data within this region
                    filecount+=1
    		#add the total number of files for the day (within bounds) to the year list
                files_per_day.append(filecount)
    ...
    plt.legend()
    plt.savefig(png_filepath + 'cdf_numfiles_landcover_Kenya_pixels_%d.png' % (int(year)))
	
main()
         #(df_bounding_box['LST'].count() == 0): #if dataframe empty or only null values in LST
